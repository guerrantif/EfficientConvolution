...
// Convolution
template <class T>
void Tensor<T>::convolve(Tensor<T>& output, const Tensor<T>& kernel, const int32_t stride, const int32_t padding, const uint32_t nThreads) const {
    // 3D image
    if(this->nElements == 1){ 
        // 3D-3D convolution
                ...
                for(auto n = 0; n < Hf; n++) {
                    for(auto k = 0; k < Wo; k++) {
                        for(auto m = 0; m < Wf; m++) {
                            ...
                            output.at(j, l, k) += inputTensorValue * kernel.at(i, n, m);
                        }
                    }
                }
            }
...
Tensor<T> Tensor<T>::convolveParallelHo(const Tensor<T>& kernel, const int32_t stride, const int32_t padding, const uint32_t nThreads) const{
    ...
    for(auto l = 0; l < Ho; l += thread_range) {
        ...
        threads.emplace_back([&, l, thread_range]() {
            this->convolveThreadHo(output, kernel, stride, padding, l, l + thread_range, Hi, Wi, Co, Ci, Wo, Hf, Wf);
        });
    }
